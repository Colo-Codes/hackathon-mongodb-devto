    ...
    return (
        <nav className="flex justify-center">
            <ul className="flex rounded-md mt-8">
                ...
                <li className="mr-4">
                    <button type="button" onClick={() => currentPage < pageNumbers.length && paginate(currentPage + 1)} className={`py-2 px-4 leading-tight bg-white border border-gray-200border-r-0 ml-0 rounded-l hover:text-white ${currentPage < pageNumbers.length ? 'text-indigo-700 hover:bg-indigo-500' : 'text-gray-700 hover:bg-gray-500 disabled:bg-gray-50 disabled:text-gray-500 disabled:border-gray-200 disabled:shadow-none'}`} disabled={'disabled'}>
                        Next page
                    </button>
                </li>
            </ul>
